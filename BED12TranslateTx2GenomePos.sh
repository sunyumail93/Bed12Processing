...
#Version: Yu H. Sun, 2018-06-05 ~ 2018-06-08

array=( "$@" )
if [ ! -n "$6" ]
then
  echo "********************************************************************************************************"
  echo "*                          Welcome to use BED12TranslateTx2GenomePos.sh !                              *"
  echo "*This is a translator of transcript locations to gemomic locations, based on the BED12 annotation      *"
  echo "*Usage: `basename $0` -i [Genes.bed12] -t [Transcript.bed] -o [Output.bed12]           *"
  echo "*    Transcript.bed: Only require the first three columns :TxName start end                            *"
  ...
  echo "*For each record in the Transcript.bed, corresponding bed12 record will be generated in Output.bed12   *"
